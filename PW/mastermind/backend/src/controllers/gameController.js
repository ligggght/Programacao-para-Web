...
export const getGameState = async (req, res) => {
  try {
    ...
    const playerNumber = game.player1.toString() === playerId ? 1 : 
                        game.player2?.toString() === playerId ? 2 : 0;
    ...
    res.status(200).json({
      ...
      opponentSecretCode: isMyTurn && game.rows.length > 0 && !game.rows[game.rows.length - 1].feedback.length ?
        (playerNumber === 1 ? game.secretCodePlayer2 : game.secretCodePlayer1) : 
        null,
    });
  } catch (err) {
